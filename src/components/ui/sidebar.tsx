...
>(
  (
      ...
      return (
        <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
          <SheetContent
            ...
            data-mobile="true"
            className="w-[--sidebar-width] bg-card p-0 text-card-foreground border-r-0 [&>button]:hidden"
            style={
              {
    ...
    return (
      <div
        ref={ref}
        className={cn("peer hidden shrink-0 md:block text-card-foreground transition-[width] duration-300 ease-in-out", 
        "data-[state=expanded]:w-[var(--sidebar-width)] data-[state=collapsed]:w-[var(--sidebar-width-icon)]",
        className)}
        data-state={state}
        {...props}
      >
        <div className="h-full w-full" data-sidebar="sidebar">
          {children}
        </div>
      </div>
    )
...
>(({ className, ...props }, ref) => {
  ...
  return (
    <div
      ref={ref}
      data-sidebar="header"
      className={cn("flex h-16 items-center border-b px-4 group-data-[collapsed]:px-0 group-data-[collapsed]:justify-center",
...
>(({ className, ...props }, ref) => {
  ...
  return (
    <div
      ref={ref}
      data-sidebar="footer"
      className={cn("flex flex-col gap-2 p-2 border-t mt-auto",
        state === "collapsed" && "p-1",
      className)}
      {...props}
    />
      ...
      <>
        {React.Children.map(children, (child, index) => {
          ...
          if (typeof child === 'string' || (React.isValidElement(child) && child.type === 'span')) { // Text
            return <span className={cn('flex-1 truncate', state === 'collapsed' && 'hidden')}>{child}</span>;
          }
          return child;
        })}
      </>
